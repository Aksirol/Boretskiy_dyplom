...
class ComputerDialog(QDialog):
    # Додали параметр computer_types (список об'єктів ComputerType з бази)
    def __init__(self, repo, computer_types, computer=None, parent=None):
    ...
    def _build_form(self):
        ...
        # Створюємо ComboBox для типу комп'ютера
        self.f_type = QComboBox()
        for ct in self.computer_types:
            self.f_type.addItem(ct.name, ct.id)  # Показуємо name, зберігаємо id
        ...
        layout.addRow("Тип ПК *", self.f_type)  # Додано нове поле
        ...
        layout.addRow("Накопичувач", self.f_storage)
    ...
    def _collect(self) -> dict:
        return {
            ...
            "computer_type_id": self.f_type.currentData(),  # Збираємо ID типу
    ...
    def _save(self):
        ...
        # Обробляємо помилку унікальності інвентарного номера
        except IntegrityError as e:
            if "UNIQUE constraint failed" in str(e.orig):
                QMessageBox.warning(self, "Помилка бази даних",
                                    "Комп'ютер з таким інвентарним номером вже існує!\nБудь ласка, введіть інший номер.")
                self.f_inventory.setFocus()
            else:
                QMessageBox.critical(self, "Помилка бази даних", f"Порушення цілісності: {str(e.orig)}")
